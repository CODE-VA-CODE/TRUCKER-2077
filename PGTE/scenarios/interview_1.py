...
def scenario_1():
    ...
    if(question == 3): # Обажаю переработки!
        ...
        bs += 1
        sc_1.show("bs", "sm_smile", bsc_tuple[0], bsc_tuple[1])
        sc_1.tell("bs", "Ну, хоть что-то полезное в тебе есть")
        sc_1.tell("na", "~ Фух... Пронесло ~")
    sc_1.show("bs", "unkindly",  bsc_tuple[0], bsc_tuple[1])
    sc_1.tell("bs", "Итак")
    sc_1.tell("bs", "Перейдем к следующему вопросу:")
    ...
    sc_1.tell("bs", "Хорошо")
    sc_1.tell("bs", "Поехали дальше")
    sc_1.tell("bs", "У тебя семья есть?")
    sc_1.tell("bs", "Жена, дети?")
    sc_1.tell("na", "~ Зачем ему знать про мою семью? ~")
    sc_1.tell("na", "~ Может он подумает что раз я без семьи то я не надежный? ~")
    question = sc_1.choice("Солгать", "Сказать правду")
    if(question == 1):
        sc_1.tell("i", "Конечно есть!")
        sc_1.tell("i", "У меня прекрасная жена и двое детей")
        sc_1.tell("bs", '"Шуршит бумажками"')
        sc_1.tell("bs", ".")
        sc_1.tell("bs", "..")
        sc_1.tell("bs", "...")
        sc_1.tell("na", "~ Что-то он долго думает ~")
        sc_1.tell("na", "~ Мне это не нравится ~")
